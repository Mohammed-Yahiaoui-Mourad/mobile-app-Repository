...
export default function ProfileScreen() {
  ...
  const { requests, mySchedules } = useBlood();
  ...
  const handleResetData = () => {
    Alert.alert(
      'Reset App State',
      'Logout and sign back in to reset your application state. Continue?',
      [
  ...
  const completedDonationsCount = mySchedules.filter((s) => s.status === 'completed').length;
      ...
      <ThemedView style={styles.container}>
        ...
        <ThemedView type="surface" style={styles.profileCard}>
          ...
          <View style={styles.profileMeta}>
            <ThemedText style={styles.profileName}>{user?.full_name || user?.name || 'Sarah Connor'}</ThemedText>
            ...
            <View style={styles.bloodTypeRow}>
              <View style={[styles.bloodTypePill, { backgroundColor: theme.primary }]}>
                <ThemedText type="smallBold" style={styles.bloodTypeText}>
                  Blood Type: {user?.donor_profile?.blood_type || user?.bloodType || 'Unknown'}
                </ThemedText>
              </View>
